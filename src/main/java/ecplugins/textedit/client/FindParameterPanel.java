...
// FindParameterPanel.java --
//
// FindParameterPanel.java is part of ElectricCommander.
//
...
public class FindParameterPanel
        ...
        ParameterPanelProvider
{
    ...
    // ~ Static fields/initializers----------------------------
    private static UiBinder<Widget, FindParameterPanel> s_binder = GWT.create(
            Binder.class);
    ...
    // ~ Instance fields
    // --------------------------------------------------------
    @UiField FormBuilder findParameterForm;
    ...
    @Override public Widget doInit()
    {
        ...
        findParameterForm.addRow(true, "Search Mode:",
        ...
        findParameterForm.addRow(true, "Find:", "Text to find in the file.",
            FIND, "", new TextBox());
        findParameterForm.addRow(true, "Search In:", "Select where to look",
            SEARCH_IN, "single", search_in);
        findParameterForm.addRow(true, "File path:", "Path to the file to use.",
            FILE_PATH, "", new TextBox());

        // File paths, List oft Paths to the files to use, separated with
        // semicolon.
        // Folder path, Path to the folder to use.
        findParameterForm.addRow(false, "Filters:",
            "Filter file names and/or extensions.", FILTERS, "*.*",
            new TextBox());
        findParameterForm.addRow(false, "Match Case:",
    ...
    {
        boolean validationStatus = findParameterForm.validate();

        // SEARCH_MODE is required.
        if (StringUtil.isEmpty(findParameterForm.getValue(SEARCH_MODE))) {
            findParameterForm.setErrorMessage(SEARCH_MODE,
                "This Field is required.");
            validationStatus = false;
        }

        // FIND is required.
        if (StringUtil.isEmpty(findParameterForm.getValue(FIND)
                                                .trim())) {
            findParameterForm.setErrorMessage(FIND, "This Field is required.");
            validationStatus = false;
        }

        // SEARCH_IN is required.
        if (StringUtil.isEmpty(findParameterForm.getValue(SEARCH_IN))) {
            findParameterForm.setErrorMessage(SEARCH_IN,
                "This Field is required.");
            validationStatus = false;
        }

        // FILE_PATH is required.
        if (StringUtil.isEmpty(findParameterForm.getValue(FILE_PATH)
                                                .trim())) {
            findParameterForm.setErrorMessage(FILE_PATH,
                "This Field is required.");
            validationStatus = false;
        }
    ...
    {
        String search = findParameterForm.getValue(SEARCH_IN);

        findParameterForm.setRowVisible(FILTERS, "folder".equals(search));
    }
    ...
    @Override public Map<String, String> getValues()
    {
        ...
        Map<String, String> findFormValues = findParameterForm.getValues();

        actualParams.put(FILE_PATH, findFormValues.get(FILE_PATH));
        actualParams.put(FILTERS, findFormValues.get(FILTERS));
        actualParams.put(FIND, findFormValues.get(FIND));
        actualParams.put(MATCH_CASE, findFormValues.get(MATCH_CASE));
        actualParams.put(SEARCH_IN, findFormValues.get(SEARCH_IN));
        actualParams.put(SEARCH_MODE, findFormValues.get(SEARCH_MODE));
            ...
            Collection<ActualParameter> actualParameters)
    {
        ...
        for (String key : new String[] {
            ...
            findParameterForm.setValue(key,
        ...
        findParameterForm.setValue(MATCH_CASE, match_case);
        updateRowVisibility();
    }
    ...
    // -------------------------------------------------------
    interface Binder
        extends UiBinder<Widget, FindParameterPanel> { }
}
